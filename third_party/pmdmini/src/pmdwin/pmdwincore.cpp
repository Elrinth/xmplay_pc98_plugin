...
void PMDWIN::psgmain(QQ *qq)
{
	...
	if(qq->lfoswi) {
		...
		if(pmdwork.lfo_switch & 0x19) {
			...
			if(!(qq == &SSGPart[2] && qq->onkai == 255 && open_work.kshot_dat && !pmdwork.ppsdrv_flag)) {
				otodasip(qq);
			}
		}
	}
...
uint8_t * PMDWIN::toneadr_calc(QQ *qq, int32_t dl)
{
	...
	if(open_work.prg_flg == 0 && qq != &EffPart) {
		...
		while(*bx != dl) {
			bx += 26;
			if(bx > mdataarea + sizeof(mdataarea) - 26) {
				return open_work.prgdat_adr + 1;	// 見つからないときは最初の音色を設定
			}
		}
...
void PMDWIN::play_init(void)
{
	...
	if(open_work.mmlbuf[*p] == 0x80) {		//先頭が80hなら演奏しない
		...
		ADPCMPart.address = &open_work.mmlbuf[*p];
	}
...
TCHAR * PMDWIN::search_pcm(TCHAR *dest, const TCHAR *filename, const TCHAR *current_dir)
{
	...
	do {
		i++;
		if(open_work.pcmdir[i][0] == filepath.EmptyChar) {
			*dest = filepath.EmptyChar;
			return dest;
		}
...
int32_t PMDWIN::music_load3(uint8_t *musdata, int32_t size, TCHAR *current_dir)
{
	...
	TCHAR	ppzfilename1[_MAX_PATH];
	TCHAR	ppzfilename2[_MAX_PATH];
	TCHAR	fullfilename[_MAX_PATH];
	bool	ppsext;
	TCHAR	*p;
	...
	if(*pcmfilename != filepath.EmptyChar) {
		if(filepath.Comparepath(pcmfilename, _T(".P86"), FilePath::extractpath_ext) == 0) {
			...
			if(resultp1 == _P86DRV_OK || resultp1 == _WARNING_P86_ALREADY_LOAD) {
				...
				filepath.ExchangeExt(pcmfilename, pcmfilename, _T(".PPC"));
				search_pcm(fullfilename, pcmfilename, current_dir);
				resultp2 = ppc_load2(fullfilename);
				if(resultp2 == PMDWIN_OK || resultp2 == WARNING_PPC_ALREADY_LOAD) {
					open_work.use_p86 = false;
																...
																// PPC -> P86
			search_pcm(fullfilename, pcmfilename, current_dir);
			resultp1 = ppc_load2(fullfilename);
	...
	if(*pcmfilename != filepath.EmptyChar) {
		// 拡張子を .PPS に変換する
		if(filepath.Comparepath(pcmfilename, _T(".PPS"), FilePath::extractpath_ext) == 0) {
			ppsext = true;
		} else {
...
int32_t PMDWIN::ppc_load3(uint8_t *pcmdata, int32_t size)
{
	...
	memcpy(&tempbuf2[sizeof(PPCHeader)-1], &pcmends, (sizeof(pcmends) <= (sizeof(tempbuf2)-(sizeof(PPCHeader)-1))) ? sizeof(pcmends) : sizeof(tempbuf2)-(sizeof(PPCHeader)-1));
...
*/
	...
	filepath.Clear(open_work.ppcfilename, sizeof(open_work.ppcfilename)/sizeof(TCHAR));
	for(i = 0; i < MAX_PCMDIR+1; i++) {
		filepath.Clear(open_work.pcmdir[i], sizeof(open_work.pcmdir[i])/sizeof(TCHAR));
	}
...
void PMDWIN::_init(void)
{
	...
	memset(&effwork, 0, sizeof(EFFWORK));
	...
	memset(mdataarea, 0x00, sizeof(mdataarea));
	memset(vdataarea, 0x00, sizeof(vdataarea));
	memset(edataarea, 0x00, sizeof(edataarea));
	memset(&pcmends, 0x00, sizeof(pcmends));
	
	// OPEN_WORK メンバの初期化
	open_work.rate = SOUND_44K;
	open_work.ppzrate = SOUND_44K;
...
bool WINAPI PMDWIN::setpcmdir(TCHAR **path)
{
	...
	while(path[i] != NULL && i < MAX_PCMDIR) {
		...
		filepath.Strcpy(open_work.pcmdir[i], path[i]);
		filepath.AddDelimiter(open_work.pcmdir[i]);
...
int32_t WINAPI PMDWIN::music_load(TCHAR *filename)
{
	...
	if(result == PMDWIN_OK ||
			...
			result == ERR_OPEN_PPS_FILE ||
			result == ERR_OPEN_PPZ1_FILE ||
			result == ERR_OPEN_PPZ2_FILE ||
			result == ERR_WRONG_PPC_FILE ||
			result == ERR_WRONG_P86_FILE ||
			result == ERR_WRONG_PPS_FILE ||
			result == ERR_WRONG_PPZ1_FILE ||
			result == ERR_WRONG_PPZ2_FILE ||
			result == WARNING_PPC_ALREADY_LOAD ||
		...
		filepath.Strcpy(open_work.mus_filename, filename);
	} else {
		open_work.mus_filename[0] = filepath.EmptyChar;
	}
	return result;
}
...
char* WINAPI PMDWIN::_getmemo3(char *dest, uint8_t *musdata, int32_t size, int32_t al)
{
	...
	_getmemo2((char *)buf, musdata, size, al);
...
char* WINAPI PMDWIN::getmemo(char *dest, uint8_t *musdata, int32_t size, int32_t al)
{
	...
	memset(buf, 0, mdata_def*1024 + 64);
	
	
	if(_getmemo(buf, musdata, size, al) != NULL) {
		strncpy(dest, buf, MAX_MEMOBUF);
		if(strlen(buf) >= MAX_MEMOBUF) {
...
char* WINAPI PMDWIN::getmemo2(char *dest, uint8_t *musdata, int32_t size, int32_t al)
{
	...
	if(_getmemo2(buf, musdata, size, al) != NULL) {
		strncpy(dest, buf, MAX_MEMOBUF);
		if(strlen(buf) >= MAX_MEMOBUF) {
			dest[MAX_MEMOBUF-1] = '\0';
		}
...
char* WINAPI PMDWIN::getmemo3(char *dest, uint8_t *musdata, int32_t size, int32_t al)
{
	...
	if(_getmemo3(buf, musdata, size, al) != NULL) {
		strncpy(dest, buf, MAX_MEMOBUF);
		if(strlen(buf) >= MAX_MEMOBUF) {
			dest[MAX_MEMOBUF-1] = '\0';
		}
...
int32_t	WINAPI PMDWIN::fgetmemo2(char *dest, TCHAR *filename, int32_t al)
{
	...
	if((rslt = _fgetmemo2(buf, filename, al)) == PMDWIN_OK) {
		strncpy(dest, buf, MAX_MEMOBUF);
		if(strlen(buf) >= MAX_MEMOBUF) {
			dest[MAX_MEMOBUF-1] = '\0';
		}
...
int32_t	WINAPI PMDWIN::fgetmemo3(char *dest, TCHAR *filename, int32_t al)
{
	...
	if((rslt = _fgetmemo3(buf, filename, al)) == PMDWIN_OK) {
		strncpy(dest, buf, MAX_MEMOBUF);
		if(strlen(buf) >= MAX_MEMOBUF) {
			dest[MAX_MEMOBUF-1] = '\0';
		}
