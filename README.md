...
VERSIONINFO FILEVERSION is **1.0.19.0**; `PLUGIN_XMPVER` is **1001900**.
...
| **MsDRV** `.MS` | ymfm (OPN/OPNA) / TSF | KENJI MSDRV4 light. `_N`=OPN, `_B2`=OPNA, `_GS`/`_88`=GS (SC-55 SF2), `_SB`=OPL3 (silent). Sibling `{prefix}.OPN`/`.SSG`. Probe by header so PMD `.MS` stays PMD. ymfm clocked via `sample_rate` (1.0.19) |
...
make dll
make scan
make pack     # dist/xmp-pc98-1.0.19.zip
make fetch-xml
```
