...
static void wrx(bgm_state *s, int ext, uint8_t aa, uint8_t dd)
{
	if (!s->opna) return;
	if (ext) {
		s->opna->write(2, aa);
...
static void note_fm(bgm_state *s, int c, int pitch)
{
	...
	fm = c - 3;
	{
		int ext = fm >= 3;
		int slot = ext ? fm - 3 : fm;
		int id = ext ? (slot + 4) : slot;
		wrx(s, ext, (uint8_t)(0xA4 + slot), (uint8_t)((blk & 0x38) | ((fnum >> 8) & 7)));
		wrx(s, ext, (uint8_t)(0xA0 + slot), (uint8_t)fnum);
...
static int skip_na_prefix(bgm_state *s, int c, const uint8_t *d, size_t n, int off)
{
	int p = off;
	if (p < 0 || p >= (int)n) return -1;
	if (d[p] == 0xF5 && p + 3 <= (int)n)
		p += 3;
	while (p + 1 < (int)n && d[p] == 0xE6) {
		int tag = d[p + 1];
		p += 2;
		if (tag == 0x5D)
			break;
		if (tag >= 0x5A && tag <= 0x5F && p + 25 <= (int)n) {
			int slot = 240 + c;
			if (slot < BGM_TONES) {
				memcpy(s->tone + slot * BGM_TONE_SZ, d + p, 25);
				s->ch[c].inst = slot;
			}
			p += 25;
		} else
...
static int parse_parts(bgm_state *s)
{
	...
	int c, got = 0;
	if (n < 0x28 || rd16(d) != 0x0026) return -1;
	memset(s->ch, 0, sizeof s->ch);
	for (c = 0; c < BGM_CH; ++c) {
		int off = (int)rd16(d + 2 + c * 2);
		int pc;
		if (off <= 0 || off >= (int)n) continue;
		if (d[off] == 0xFC) continue;
		pc = skip_na_prefix(s, c, d, n, off);
		if (pc < 0 || pc >= (int)n) continue;
		s->ch[c].enabled = 1;
		s->ch[c].pc = pc;
		s->ch[c].base = 0;
		s->ch[c].wait = 8;
...
{
	uint32_t sr;
	if (!data || len < 0x28 || !pc98_looks_na(data, len))
		return -1;
		...
		const pc98_cfg *cfg, pc98_info *out)
{
	...
	if (!pc98_looks_na(data, len)) return -1;
	...
	out->kind = PC98_KIND_NA;
	out->songs = 1;
	title_from_path(out->title, sizeof out->title, filename);
	bounded(out->filetype, sizeof out->filetype, "NA");
	bounded(out->engine, sizeof out->engine, "ymfm Packen NA");
