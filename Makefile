...
# /usr/bin/make pack     # xmp-pc98-1.0.25.zip
...
pack: dll
	...
	rm -f $(DIST)/xmp-pc98-1.0.25.zip
	powershell.exe -NoProfile -Command \
		"Compress-Archive -Path '$(DIST)/pack/*' -DestinationPath '$(DIST)/xmp-pc98-1.0.25.zip' -Force"
	rm -rf $(DIST)/pack
	ls -l $(DIST)/xmp-pc98.dll $(DIST)/xmp-pc98-1.0.25.zip

clean:
	rm -rf $(DIST)/xmp-pc98.dll $(DIST)/pc98-scan $(DIST)/pc98-scan.exe \
		$(DIST)/test_s98_render $(DIST)/test_pmd_render $(DIST)/test_pc98_render \
		$(DIST)/obj $(DIST)/obj-i686 $(DIST)/pack $(DIST)/xmp-pc98-1.0.25.zip
